...
type 'elm tree =
  | Lf (* Leaf *)
  | Br of { (* Branch *)
            left : 'elm tree; value : 'elm; right : 'elm tree }
...
let rec reflect t =
  ...
  | Br { left = l; value = v; right = r } ->
      Br { left = reflect r; value = v; right = reflect l }

let rec add (t, e) =
  match t with
  | Lf -> Br { left = Lf; value = e; right = Lf }
  | Br { left = l; value = v; right = r } ->
      Br { left = add (l, e); value = v; right = r }
...
let rec string_of_string_tree t =
  match t with
  | Lf -> "leaf"
  | Br { left = l; value = v; right = r } ->
      "branch(" ^ string_of_string_tree l ^ "," ^ v ^ ","
      ^ string_of_string_tree r ^ ")"

let rec treemap f t =
  match t with
  | Lf -> Lf
  | Br { left = l; value = v; right = r } ->
      Br { left = treemap f l; value = f v; right = treemap f r }
